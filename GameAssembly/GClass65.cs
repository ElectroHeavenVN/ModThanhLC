...
public class GClass65
{
	...
	{
		GClass73.smethod_31(mResources.downloading_data);
	}

	public static int smethod_1(int max)
	{
		return new GClass2().method_1(max);
	}

	public static sbyte[] smethod_2(GClass208 msg)
	{
		try
		{
	...
	public static sbyte[] smethod_3(GClass100 dos)
	{
		try
		{
		...
		{
			GClass36.smethod_2("LOI DOC readByteArray dos  NINJAUTIL");
		}
		return null;
	}
	...
	public static string smethod_9(long m)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			{
				long num2 = m % 1000L;
				text = ((num2 == 0L) ? (".000" + text) : ((num2 < 10L) ? (".00" + num2 + text) : ((num2 < 100L) ? (".0" + num2 + text) : ("." + num2 + text))));
				m /= 1000L;
				continue;
			}
			text = m + text;
