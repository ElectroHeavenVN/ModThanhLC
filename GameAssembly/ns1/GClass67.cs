...
namespace ns1
{
	public class GClass67 : GInterface5
	{
		...
		private SocksAuthentication enum0_0;
		...
		public GClass67(TcpClient tcpClient)
		{
			if (tcpClient == null)
		...
		public GClass67(string proxyHost)
		{
		...
		public GClass67(string proxyHost, int proxyPort)
		{
			if (!string.IsNullOrEmpty(proxyHost))
			{
		...
		public GClass67(string proxyHost, string proxyUserName, string proxyPassword)
		{
			if (string.IsNullOrEmpty(proxyHost))
				throw new ArgumentNullException("proxyHost");
		...
		public GClass67(string proxyHost, int proxyPort, string proxyUserName, string proxyPassword)
		{
			if (!string.IsNullOrEmpty(proxyHost))
			{
				if (proxyPort <= 0 || proxyPort > 65535)
		...
		public TcpClient CreateConnection(string destinationHost, int destinationPort)
		{
			if (!string.IsNullOrEmpty(destinationHost))
			{
				...
				{
					throw new GException0(string.Format(CultureInfo.InvariantCulture, "Connection to proxy host {0} on port {1} failed.", Class17.smethod_0(tcpClient_0), Class17.smethod_1(tcpClient_0)), innerException);
				}
			}
		...
		private void method_0()
		{
			if (string_1 != null && string_2 != null)
				enum0_0 = SocksAuthentication.UsernamePassword;
			else
				enum0_0 = SocksAuthentication.None;
		}
		...
		private void method_1()
		{
			...
			if (b != byte.MaxValue)
			{
				if (b == 2 && enum0_0 == SocksAuthentication.None)
				{
					tcpClient_0.Close();
