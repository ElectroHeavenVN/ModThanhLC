	...
	public static class SocketServer
	{
		...
		private static void onMessage(vMessage msg, StateObject state)
		{
			try
			{
				...
				try
				{
					state.account = waitingAccounts.Find(x => x.Username == Encoding.ASCII.GetString(msg.data));
					if (state.account != null)
					{
		...
		public static void ReadCallback(IAsyncResult ar)
		{
			...
			if (stateObject.account != null)
			{
				...
				stateObject.account.status = ((MainController.language == 0) ? "Mất kết nối" : "Disconnected");
			}
		}
