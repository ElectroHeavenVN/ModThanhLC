...
namespace DragonBoyManager
{
    public partial class MainController : Form
	{
		...
		public static void ArrangeWindows(List<Account> accounts, int type)
		{
			...
			accounts.RemoveAll(predicate ?? (predicate = (Account x) => x.process == null || x.process.HasExited));
		...
		public async Task openAccount(Account account)
		{
			if (account.process == null || account.process.HasExited)
			{
				...
				if (account.isUseProxy && Options[1] == "T")
					account.process.StartInfo.Arguments = $"--username {account.Username.Trim()} --password {account.Password.Trim()} --server {account.Server.ToString().ToLower().Replace(" ", "")} --options {Options[0] + "|" + Options[1] + "|" + Options[2] + "|" + Options[3]} --isUseProxy {account.isUseProxy.ToString()} --proxy {account.ProxyInfo.Trim()} --size {account.SizeScreen.Trim()} --uuid {"username:" + CheckInfo.t1 + "," + CheckInfo.t.Trim()}";
				else
					account.process.StartInfo.Arguments = $"--username {account.Username} --password {account.Password} --server {account.Server.ToString().ToLower().Replace(" ", "")} --options {Options[0] + "|" + Options[1] + "|" + Options[2] + "|" + Options[3]} --isUseProxy {account.isUseProxy.ToString()} --size {account.SizeScreen} --uuid {"username:" + CheckInfo.t1 + "," + CheckInfo.t}";
		...
		private void update_Tick(object sender, EventArgs e)
		{
			try
			{
				...
				if (!groupBox1.Enabled)
					return;
				if (float.Parse(VERSION) >= float.Parse(CheckInfo.VERSION) && Opacity < 100.0 && currentTimeMillis() - TimePlus > 2000L)
				{
					Opacity += 1.0;
					TimePlus = currentTimeMillis();
				}
		...
		private void tabData1_Load(object sender, EventArgs e)
		{
		}
